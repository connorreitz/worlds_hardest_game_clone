...
void waitForVBlank(void) {
  ...
  while (SCANLINECOUNTER > 160 && SCANLINECOUNTER <= 228) {
    continue;
  }
...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for (int i = 0; i < height; i++) {
    ...
    DMA[3].dst = &videoBuffer[OFFSET(row + i + 1, col, 240)];
  }
}
...
void colorIn(int row, int col, int sideLength, const u16 *image) {
    DMA[3].src = image + OFFSET(row, col, 240);
    DMA[3].dst = &videoBuffer[OFFSET(row, col, 240)];
    for (int i = 0; i < sideLength; i++) {
        DMA[3].cnt = (sideLength) | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
        DMA[3].dst = &videoBuffer[OFFSET(row + i + 1, col, 240)];
        DMA[3].src = image + OFFSET(row + i + 1, col, 240);
